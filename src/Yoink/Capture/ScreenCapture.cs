...
public static class ScreenCapture
{
    ...
    /// <summary>Draws the cursor onto a device context (BitBlt destination), avoiding GDI+ bitmap allocations per frame.</summary>
    private static void DrawCursor(IntPtr hdcDest, Rectangle captureBounds)
    {
        var cursorInfo = new User32.CURSORINFO
        {
            cbSize = Marshal.SizeOf<User32.CURSORINFO>()
        ...
        try
        {
            int x = cursorInfo.ptScreenPos.X - captureBounds.X - (int)iconInfo.xHotspot;
            int y = cursorInfo.ptScreenPos.Y - captureBounds.Y - (int)iconInfo.yHotspot;
            User32.DrawIconEx(hdcDest, x, y, cursorInfo.hCursor, 0, 0, 0, IntPtr.Zero, User32.DI_NORMAL);
        }
        finally
        {
            if (iconInfo.hbmMask != IntPtr.Zero)
    ...
    /// <summary>
    /// Reuses one bitmap, graphics, and screen DC for the entire recording — avoids per-frame GDI+ allocation during screen recording.
    /// </summary>
